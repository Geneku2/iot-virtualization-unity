...
if __name__ == "__main__":
	...
	while True:
		...
		if request == b"START":
			...
			states = [0 for i in range(1, ele_cnt + lion_cnt + zebra_cnt + 1) ]
		...
		# 	ready = 1
			...
			for i in range(len(pos)):
				if states[i] != 2:
					states[i] = int(eat_trigger)
					if random.random() < 0.001:
						states[i] = 2
				if(i<ele_cnt):
					animalArr.append({"id":i, "type":0, "state":states[i], "position":{"x": pos[i][0], "z": pos[i][1]}, "velocity":{}})
				elif(i<lion_cnt+ele_cnt):
					animalArr.append({"id":i, "type":1, "state":states[i], "position":{"x": pos[i][0], "z": pos[i][1]}, "velocity":{}})
				else:
					animalArr.append({"id":i, "type":2, "state":states[i], "position":{"x": pos[i][0], "z": pos[i][1]}, "velocity":{}})
